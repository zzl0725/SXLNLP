...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            mask = torch.tril(torch.ones((x.shape[0], x.shape[1], x.shape[1]))) # 下三角矩阵 batch_size * seq_len * seq_len
            if torch.cuda.is_available():
                mask = mask.cuda()
            x, _= self.bert(x, attention_mask=mask)  #output shape:(batch_size, seq_len, hidden_size)
...
def train(corpus_path, save_weight=True):
    ...
    pretrain_model_path = r"D:\2024 AILearning\BaDou_Course\Practical_Project\生成式任务\bert-base-chinese"
