...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    pred_score = 0
    count = 0
    for user_id，moive_id, similarity in similar_items[moive_id][:topn]:
        #对这部电影的打分相似的电影
        rating_by_similiar_item = user_to_rating[similar_items][item_id - 1]
...
#对于一个用户做完整的item召回
def movie_recommand(user_id, similar_items, user_to_rating, item_to_name, topn=10):
    #当前用户还没看过的所有电影id
