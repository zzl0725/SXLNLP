...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    # similar_items 是每个物品和其他物品的相似度 similar_items[i]==99个物品index和相似度
    # user_to_rating 是每个用户给每个电影所打的分数
    # 对于用户喜欢的物品，计算其他物品的相似度，然后加权平均
    # 先找到用户评分最高的10个电影
    pred_score = 0
    count = 0
    like_item_id_list = user_to_rating[user_id]
    like_item_id_map = {}
    for item_id1, rating in enumerate(like_item_id_list):
        if rating != 0:
            like_item_id_map[item_id1+1] = rating

    # 然后计算这些电影和给定电影的相似度，然后加权平均
    for like_item_id, score in like_item_id_map.items():
        # 物品之间的相似度
        similarity_item_list = similar_items[like_item_id]
        # 找到和给定item的分数
        similarity = 0
        for item_id2, sim in similarity_item_list:
            if item_id2 == item_id:
                similarity= sim
                break
        pred_score += score * similarity
        count += 1
    pred_score /= count + 1e-5
    return pred_score
