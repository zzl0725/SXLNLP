...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    for similar_item, similarity in similar_items[item_id]:
        #同一用户对不同电影的打分
        rating_by_similiar_item = user_to_rating[user_id][similar_item]
        #分数*电影相似度，作为一种对分数的加权，越相似的电影评分越重要
        pred_score += rating_by_similiar_item * similarity
    pred_score /= (count + 1e-5) * (1e+6)
    return pred_score
...
if __name__ == "__main__":
    ...
    while True:
        user_id = int(input("输入用户id："))
        item_id = int(input("输入电影id："))
        res = item_cf(user_id, item_id, similar_items, user_to_rating)
        print(res)
