...
class DialogSystem:
    ...
    def replace_slot(self, text, memory):
        hit_intent = memory["hit_intent"]
        if "slot" not in self.node_id_to_node_info[hit_intent]:
            return text
        else:
            slots = self.node_id_to_node_info[hit_intent].get("slot", [])
            for slot in slots:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for node_i in ds.node_id_to_node_info.values():
            # print(node_i["intent"])
            if node_i["intent"] == []:
                # 插入用户输入
                node_i["intent"].insert(0, user_input)
                # print(node_i["intent"][0])
                user_input = node_i["intent"][0]
