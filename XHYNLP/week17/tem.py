...
def item_cf(user_id, item_id, item_to_similar_item, user_to_rating, topn=10):
    ...
    # similar_items = item_to_similar_item[item_id][:topn]
    for similar_item, similarity in item_to_similar_item[item_id]:
        # 当前用户对相似项目的评分
    ...
    # 如果没有任何相似项目被用户评分过，则返回默认评分（例如平均评分）
    if similarity_sum == 0:
        return 0  # 或者可以返回该用户的所有评分的平均值等
    # 加权平均
    pred_score /= similarity_sum
    return pred_score
...
def movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10):
    ...
    ##itemid需要在100以内，因为之前代码的限制。
    unseen_items = [item_id for item_id in unseen_items if item_id < 100 and item_id in similar_items]
    res = []
