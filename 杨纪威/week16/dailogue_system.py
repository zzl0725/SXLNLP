...
class DialogSystem:
    ...
    def load(self):
        ...
        self.load_scenario("买衣服.json")
        self.load_scenario("看电影.json")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if user_input in ["没听清楚","再说一遍","重听", "重新来"]:
            if memory.get("user_input") is None:
                print("您还没问任何问题，不支持重听。")
            else:
                print("user_query:", memory.get("user_input", "No content"))
                print("bot_response:", memory.get("bot_response", "No content"))
        else:
