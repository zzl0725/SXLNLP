# -*- encoding: utf-8 -*-
"""
movie_recommand_week17_hw_bylyu.py
Created on 2024/11/5 21:30
@author: Allan Lyu
@version: v1.0
@Description: 第17周作业（基于电影打分数据集, 使用item_cf实现协同过滤）
"""
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    for similar_item, similarity in similar_items[item_id][:topn]:
        # 获取用户对这些相似物品的评分
        rating_by_user = user_to_rating[user_id][similar_item - 1]  # 物品id从1开始，所以减1
        if rating_by_user != 0:  # 如果用户看过该物品，则计算加权评分
            pred_score += rating_by_user * similarity
            count += 1

    # 如果用户没有评分任何相似的物品，则返回默认分数0
    if count == 0:
        return 0
    else:
        # 计算预测评分
...
if __name__ == "__main__":
    ...
    # lyu： 添加注释打印相似用户和相似物品
    # print("相似用户计算完成%s"%(similar_user))
    # print("相似物品计算完成%s"%(similar_items))
