...
class DialogSystem:
    ...
    def load(self):
        ...
        self.load_scenario("scenario-通用.json")
    ...
    def get_intent(self, memory):
        ...
        max_score = -1
        hit_intent = None
        memory["available_nodes"].append("scenario-通用-node1")
    ...
    def policy(self, memory):
        ...
        if memory["need_slot"] is None:
            hit_intent = memory["hit_intent"]
            hit_node = self.node_id_to_node_info[hit_intent]
            if hit_node.get("action", []) == "REPEAT":
    ...
    def nlg(self, memory):
        ...
        if memory["action"] == "answer":
            ...
            memory["bot_response"] = memory.get("bot_response", "您还没说呢")
        else:
            #反问
