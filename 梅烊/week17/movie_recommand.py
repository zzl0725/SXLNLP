...
def find_similar_user(user_to_rating):
    ...
    for user_a, ratings_a in user_to_rating.items():
        ...
        for user_b, ratings_b in user_to_rating.items():
            #全算比较慢，省去一部分用户, or user_b > 100 or user_a > 100
            if user_b == user_a:
                continue
            #ab用户互换不用重新计算cos
...
#自己尝试实现
# 计算相似item的得分
# 输入：user_id, item_id, 相似item列表，用户打分矩阵
# 输出：预测打分
def item_cf(user_id, item_id, similar_items, user_to_rating):
    pred_score = 0
    count = 0
    # 取出user_id看过的电影、分数集合
    seen_items = [(v_item_id + 1,rating) for v_item_id, rating in enumerate(user_to_rating[user_id]) if rating!= 0]
    # 依据用户user_id看过的电影集合与item_id的相似度，计算预测打分
    for seen_item, rating in seen_items:
        # 取出item_id与seen_item的相似度
        similar_items_score = [rating for seen_item_id, rating in similar_items[item_id] if seen_item_id == seen_item]
        # 分数*相似度，作为一种对分数的加权，越相似评分越重要
        if len(similar_items_score):
            pred_score += rating * similar_items_score[0]
            count += 1
    pred_score /= count + 1e-5
    return pred_score
