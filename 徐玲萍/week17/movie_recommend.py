...
def cosine_distance(vector1, vector2):
    ab = vector1.dot(vector2) #点乘
    a_norm = np.sqrt(np.sum(np.square(vector1))) #np.square(vector1): 这个函数计算 vector1 中每个元素的平方
    b_norm = np.sqrt(np.sum(np.square(vector2))) #np.sum(...): 这个函数计算了 vector1 中所有元素平方的和
    return ab / (a_norm * b_norm) #np.sqrt(...): 这个函数计算其参数的平方根
...
def find_similar_user(user_to_rating):
    ...
    score_buffer = {}
    for user_a, ratings_a in user_to_rating.items(): #计算user_a和其他user的相似度
        similar_user = []
...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    pred_score = 0
    count = 0
    rating = user_to_rating[user_id]
    similarity_score = 0
    for i in range(len(rating)):
        if rating[i] == 0:
            continue
        seen_item_id = i + 1
        similarities = similar_items[seen_item_id]
        for item in similarities:
            if item[0] == item_id:
                similarity_score = item[1]
                break
        pred_score += rating[i] * similarity_score
        count += 1
    pred_score /= count + 1e-5
    return pred_score
...
if __name__ == "__main__":
    user_item_score_data_path = "u.data"
    item_name_data_path = "u.item"
