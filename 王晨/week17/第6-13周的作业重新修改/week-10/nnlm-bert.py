...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(r"E:\AI\课程资料\bert-base-chinese", return_dict=False)
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            if torch.cuda.is_available():
                mask = mask.cuda()
            x, _ = self.bert(x, attention_mask=mask, return_dict=False)
            y_pred = self.classify(self.dropout(x))   #output shape:(batch_size, vocab_size)
...
def build_sample(tokenizer, window_size, corpus):
    ...
    y = tokenizer.encode(target, add_special_tokens=False, truncation=True, max_length=window_size, padding="max_length")
    return x, y
...
def train(corpus_path, save_weight=True):
    ...
    tokenizer = BertTokenizer.from_pretrained(r'E:\AI\课程资料\bert-base-chinese')
    if torch.cuda.is_available():
        model = model.cuda()
