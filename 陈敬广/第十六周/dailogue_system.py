...
class DialogSystem:
    ...
    def get_intent(self, memory):
        ...
        for node_id in memory["available_nodes"]:
            ...
            key_intent, value_score = next(iter(score.items()))
            if value_score > max_score:
                max_score = value_score
                hit_intent = node_id
                hit_key_intent = key_intent
        memory["hit_intent"] = hit_intent
        memory["hit_key_intent"] = hit_key_intent
    ...
    def get_node_score(self, node, memory):
        ...
        for node_intent in node_intents:
            ...
            intent_score[node_intent] = sentence_similarity
            scores.append(intent_score)
        return max(scores, key=lambda d: next(iter(d.values()), float('-inf')))
