...
class DialogSystem:
    ...
    def load(self):
        ...
        self.load_scenrio("scenario-订餐.json")
    ...
    def init_repeat_node(self):
        ...
        buy_cloth_node1 = "scenario-买衣服-node1"
        order_food_node1 = "scenario-订餐-node1"

        new_nodes = [buy_cloth_node1, order_food_node1, node_id]
        for node_info in self.all_node_info.values():  # 成为每个已有节点的子节点
            node_info["childnode"] = node_info.get("childnode", []) + new_nodes

    def init_memory(self):
        memory = {}
        memory["available_node"] = ["scenario-买衣服-node1", "scenario-订餐-node1"]
        return memory
